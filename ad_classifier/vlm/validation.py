...
def validate_vlm_output(
    ...
    *,
    ...
    if me.brand and me.brand.name:
        ...
        if cleaned:
            if evidence_blob and not _brand_in_evidence(cleaned, evidence_blob):
                me.brand.name = None
...
def _brand_in_evidence(brand: str, evidence_blob: str) -> bool:
    ...
    if brand_lower in evidence_blob:
        return True
...
def _product_in_evidence(product: str, evidence_blob: str) -> bool:
    ...
    if product_lower in evidence_blob:
        return True
...
def _filter_non_vehicle_prices(prices: list, evidence_blob: str, category: str = "") -> list:
    if not evidence_blob:
        return prices
    if category.lower() not in _AUTOMotive_CATEGORIES:
        return prices
