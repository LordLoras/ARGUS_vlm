...
def test_upsert_and_get_text(tmp_path):
    ...
    assert len(result) == 8
    assert all(abs(a - b) < 1e-5 for a, b in zip(vec, result, strict=True))
...
def test_upsert_overwrites_text(tmp_path):
    ...
    conn.commit()
    result = store.get_text("ad_1")
    assert all(abs(a - b) < 1e-5 for a, b in zip(new_vec, result, strict=True))
...
def test_get_frame_vectors_batch_round_trips(tmp_path):
    conn, store = _store(tmp_path, visual_dim=8)
    conn.execute(
        "INSERT INTO ads (id, source_path, ingested_at) VALUES (?, ?, ?)",
        ("ad_frame", "/tmp/ad.mp4", "2026-01-01T00:00:00"),
    ...
    emb = MockImageEmbedder(dim=8)
    first = emb.embed(Path("/img/first.png"))
    second = emb.embed(Path("/img/second.png"))
    store.upsert_frame_visual("ad_frame", 1, 500, first)
    store.upsert_frame_visual("ad_frame", 2, 1000, second)
    conn.commit()

    results = store.get_frame_vectors_batch(
        [
            SqliteVecStore.frame_key("ad_frame", 1),
            SqliteVecStore.frame_key("ad_frame", 2),
        ]
    )

    assert set(results) == {"ad_frame:1", "ad_frame:2"}
    assert all(abs(a - b) < 1e-5 for a, b in zip(first, results["ad_frame:1"], strict=True))
