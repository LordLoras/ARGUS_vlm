...
- Work must happen on `experimental`. If the repository already uses the typo branch name `experimetal`, use that exact branch instead. If neither branch exists, create `experimental` from the current `main` tip before making changes.
- Do not commit or push this experimental work to `main`. Do not merge it into the submitted-demo branch until explicitly approved.
- The UI side panel must visually and semantically separate `Submitted Past Submission` routes from `Experimental` routes. Existing submitted demo routes must remain intact.
- Product entity features must use a separate experimental graph database/store. They may read existing submitted ad records from the existing SQLite database through read-only connections, views, or adapters, but must not mutate submitted ad, classification, marketing, campaign, or agent records directly.
- Keep the implementation local-first. Prefer a separate local SQLite-backed graph store unless an explicitly approved local graph database is added.
- The graph model must support product, brand, parent-company, category, and taxonomy-mapping nodes, with typed relationships between products, brands, parent companies, categories, taxonomy mappings, and ad observations.
- Taxonomy mappings are many-to-many and confidence-aware, with provenance/source metadata.
- Weak, obscure, or low-confidence mentions must remain candidate entities until confirmed. They must not pollute the confirmed graph.
- Search discovery and third-party lookup results are candidate signals only. They are not authoritative source evidence by themselves.
- Product descriptions must be synthesized from grounded facts and provenance. Do not copy descriptions verbatim from third-party pages.
- Product pages must expose canonical name, synthesized description, brand, owner, category, taxonomy mappings, aliases, evidence/provenance, candidate or confirmed status, and related ads.
- Tests must cover graph persistence, read-only access to submitted database records, taxonomy mapping confidence, candidate vs confirmed entities, and frontend route separation.
